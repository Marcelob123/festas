let festas = JSON.parse(localStorage.getItem('festas_kenia_final')) || [];
let catalogoServicos = JSON.parse(localStorage.getItem('serv_kenia_final')) || [];
let catalogoCombos = JSON.parse(localStorage.getItem('combos_kenia_final')) || [];
let clientesSalvos = JSON.parse(localStorage.getItem('cli_kenia_final')) || []; 
...
function mudarAba(idAba, idBotao) {
    ...
    if(idAba !== 'aba-pedido' && editIndex !== null) {
        ...
        document.getElementById('titulo-vender').innerText = '🛒 PDV - Nova Venda';
...
function abrirModalClientes() {
    const ul = document.getElementById('lista-clientes-modal');
    ul.innerHTML = '';
    if(clientesSalvos.length === 0) {
        ul.innerHTML = '<li>Nenhum cliente salvo.</li>';
    } else {
        clientesSalvos.forEach((c, idx) => {
            ul.innerHTML += `<li style="padding: 10px; border-bottom:1px solid #ddd;">${c} <button class="btn-remover" onclick="removerCliente(${idx})">X</button></li>`;
        });
    }
    document.getElementById('modal-clientes').style.display = 'flex';
}
function fecharModalClientes() { document.getElementById('modal-clientes').style.display = 'none'; }
function removerCliente(idx) {
    if(confirm("Remover este cliente da lista?")) {
        clientesSalvos.splice(idx, 1);
        localStorage.setItem('cli_kenia_final', JSON.stringify(clientesSalvos));
...
function editarFesta(index) {
    ...
    document.getElementById('ped-frete').value = f.frete > 0 ? f.frete : '';
    
    pedidoAtualItens = f.itens ? [...f.itens] : [];
    gastosDetalhadosTemp = f.gastos ? [...f.gastos] : [];
    
    renderizarGastosTemp();
    atualizarTelaPedido();
    
    document.getElementById('titulo-vender').innerText = `✏️ Editando: ${f.cliente}`;
...
function renderizarGastosTemp() {
    const ul = document.getElementById('lista-gastos-detalhados'); ul.innerHTML = '';
...
function abrirModalGastos() {
    ...
    const ul = document.getElementById('lista-gastos-modal'); ul.innerHTML = ''; let total = 0;
    ...
    if(total === 0) ul.innerHTML = '<li style="padding: 1rem; text-align: center;">Nenhum gasto registrado neste mês.</li>';
    else ul.innerHTML += `<li style="background:#fecdd3; color:#e11d48; justify-content:space-between; padding: 1rem; margin-top: 10px; border-radius: 8px;"><strong>Total Gasto:</strong> <strong>${formatarMoeda(total)}</strong></li>`;
...
function atualizarVisaoGeral() {
    ...
    lista.innerHTML = ''; listaReceber.innerHTML = '';
    let dadosMensais = {};
    ...
    festas.sort((a, b) => new Date(a.data) - new Date(b.data)).forEach((festa, index) => {
        const valorTotal = parseFloat(festa.valor) || 0;
        const custo = parseFloat(festa.custo) || 0;
        const antecipacao = parseFloat(festa.antecipacao) || 0;
        ...
        if (festa.data && festa.data.startsWith(mesSelecionado)) {
            fatMes += valorTotal; custoMes += custo; qtdFestasMes++;
        }
    ...
    document.getElementById('texto-relatorio-mensal').innerHTML = `
        ...
        • Lucro Líquido: <b style="color:#059669">${formatarMoeda(fatMes - custoMes)}</b><br>
        <span style="color:#7c3aed; display:block; margin-top:4px;"><b>Aviso:</b> ${qtdFestasMes === 0 ? '⚠️ Nenhuma festa.' : `✨ ${qtdFestasMes} evento(s).`}</span>
    `;

    renderizarCalendario(); renderizarGrafico(dadosMensais);
}

function quitarFesta(index) {
    if(confirm(`Confirmar o pagamento restante e dar baixa na festa de ${festas[index].cliente}?`)){
        festas[index].antecipacao = festas[index].valor;
        festas[index].status = 'Pago';
        localStorage.setItem('festas_kenia_final', JSON.stringify(festas));
...
function renderizarGrafico(dadosMensais) {
    ...
    chartInstance = new Chart(ctx, {
        type: 'bar',
        data: { labels: labelsAmigaveis, datasets: [{ label: 'Faturamento', data: dataFaturamento, backgroundColor: '#3b82f6', borderRadius: 4 }, { label: 'Lucro', data: dataLucro, backgroundColor: '#10b981', borderRadius: 4 }] },
...
function alternarStatus(index) {
    ...
    localStorage.setItem('festas_kenia_final', JSON.stringify(festas)); atualizarVisaoGeral(); renderizarHistorico();
}

function excluirFesta(index) {
    if(confirm('Tem certeza que deseja excluir?')) { 
        festas.splice(index, 1); localStorage.setItem('festas_kenia_final', JSON.stringify(festas)); 
        atualizarVisaoGeral(); renderizarHistorico(); 
    }
}
...
function renderizarHistorico() {
    ...
    festas.sort((a, b) => new Date(b.data) - new Date(a.data)).forEach((festa, index) => {
        const classeStatus = festa.status === 'Pago' ? 'status-pago' : 'status-pendente';
        tbody.innerHTML += `<tr><td>${formatarDataCurta(festa.data)}</td><td><strong>${festa.cliente}</strong></td><td><span class="${classeStatus}" onclick="alternarStatus(${index})" style="cursor:pointer;">${festa.status}</span></td><td><button class="btn-editar" onclick="editarFesta(${index})" title="Editar">✏️</button> <button class="btn-pdf" onclick="abrirDetalhesFesta(${index})" title="Ver Relatório">📄</button></td></tr>`;
    });
}
function abrirDetalhesFesta(index) {
    const f = festas[index]; const lucro = f.valor - f.custo;
    let htmlItens = f.itens && f.itens.length > 0 ? f.itens.map(i => `<li>- ${i.nome}: ${formatarMoeda(i.valor)}</li>`).join('') : '<li>Nenhum item.</li>';
    let htmlGastos = f.gastos && f.gastos.length > 0 ? f.gastos.map(g => `<li>- ${g.qtd}x ${g.nome}: <span style="color:#ef4444">${formatarMoeda(g.total)}</span></li>`).join('') : '<li>Nenhum gasto.</li>';
    const freteDisplay = f.frete > 0 ? `<p><strong>🚚 Frete:</strong> ${formatarMoeda(f.frete)}</p>` : '';

    document.getElementById('detalhes-festa-conteudo').innerHTML = `
        <p><strong>👤 Cliente:</strong> ${f.cliente}</p>
        <p><strong>📅 Data/Hora:</strong> ${formatarDataCompleta(f.data)} às ${f.horario || 'Não definido'}</p>
        ...
        <p><strong>💳 Pago (Sinal/Baixa):</strong> ${formatarMoeda(f.antecipacao)}</p>
...
function renderizarCalendario() {
    ...
    for (let i = 1; i <= new Date(anoAtual, mesAtual + 1, 0).getDate(); i++) {
        ...
        if (festasDoDia.length > 0) grid.innerHTML += `<div class="dia dia-com-festa" onclick="abrirModalDia('${dataFormatada}')">${i} <br><small>(${festasDoDia.length})</small></div>`;
        else grid.innerHTML += `<div class="dia" onclick="abrirModalDia('${dataFormatada}')">${i}</div>`;
    }
}
...
function abrirModalDia(data) {
    ...
    const container = document.getElementById('container-festas-do-dia'); container.innerHTML = '';
    ...
    } else { container.innerHTML = `<p style="font-size:0.85rem; color:#888;">Nenhuma festa neste dia.</p>`; }
...
function irParaVender() { const dataSel = document.getElementById('modal-data').value; fecharModalDia(); mudarAba('aba-pedido', 'btn-tab-pedido'); document.getElementById('ped-data').value = dataSel; }

// CATÁLOGO
function atualizarInterfaceCatalogo() {
    const ulServicos = document.getElementById('lista-catalogo-servicos'); ulServicos.innerHTML = '';
    catalogoServicos.forEach((s, idx) => { ulServicos.innerHTML += `<li><div class="produto-info"><span class="produto-nome">${s.nome}</span><span class="badge-preco">${formatarMoeda(s.valor)}</span></div><button class="btn-remover" onclick="removerServico(${idx})">X</button></li>`; });
    ...
    catalogoCombos.forEach((c, idx) => { ulCombos.innerHTML += `<li><div class="produto-info"><span class="produto-nome">${c.nome}</span><span class="badge-preco">${formatarMoeda(c.valorVenda)}</span></div><button class="btn-remover" onclick="removerCombo(${idx})">X</button></li>`; });
...
function salvarServico() {
    ...
    catalogoServicos.push({ id: Date.now().toString(), nome, valor }); localStorage.setItem('serv_kenia_final', JSON.stringify(catalogoServicos));
    document.getElementById('cat-serv-nome').value = ''; document.getElementById('cat-serv-valor').value = ''; atualizarInterfaceCatalogo();
}
function removerServico(idx) { catalogoServicos.splice(idx,1); localStorage.setItem('serv_kenia_final', JSON.stringify(catalogoServicos)); atualizarInterfaceCatalogo(); }
...
function renderComboTemp() {
    ...
    comboTempItens.forEach(item => { ul.innerHTML += `<li><span>${item.qtd}x ${item.nome}</span></li>`; });
}
function salvarComboFinal() {
    ...
    localStorage.setItem('combos_kenia_final', JSON.stringify(catalogoCombos));
...
function removerCombo(idx) { catalogoCombos.splice(idx,1); localStorage.setItem('combos_kenia_final', JSON.stringify(catalogoCombos)); atualizarInterfaceCatalogo(); }
...
function atualizarTelaPedido() {
    ...
    pedidoAtualItens.forEach((item, idx) => {
        subtotal += parseFloat(item.valor) || 0;
    ...
    // Adiciona 50% automatico apenas se não for edição e se o campo não estiver sendo digitado
    if (!inputAnt.value && totalGeral > 0 && editIndex === null && document.activeElement !== inputAnt) {
...
function calcularRestantePedido() {
    const subtotal = pedidoAtualItens.reduce((acc, item) => acc + parseFloat(item.valor), 0);
    const frete = parseFloat(document.getElementById('ped-frete').value) || 0;
    const totalGeral = subtotal + frete;
...
function fecharPedido() {
    ...
    let custoEstimado = gastosDetalhadosTemp.reduce((acc, item) => acc + parseFloat(item.total), 0);

    if(!cliente || !data || pedidoAtualItens.length === 0) return alert('Preencha Cliente, Data e adicione itens (produtos) no passo 2!');

    let faturamentoTotal = pedidoAtualItens.reduce((acc, item) => acc + parseFloat(item.valor), 0) + frete;

    const statusFinal = (antecipacao >= faturamentoTotal && faturamentoTotal > 0) ? 'Pago' : 'Pendente';

    const novaFesta = { 
        cliente, data, horario, endereco, obs, 
        valor: faturamentoTotal, frete, custo: custoEstimado, antecipacao, 
        status: statusFinal, 
        itens: [...pedidoAtualItens], gastos: [...gastosDetalhadosTemp]
    };
...
function importarBackup(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            // Aceita festas antigas sob outras chaves (festas_v2, festas_pro, etc)
            if(dados.festas) localStorage.setItem('festas_kenia_final', JSON.stringify(dados.festas));
            else if (dados.festasAgendadas) localStorage.setItem('festas_kenia_final', JSON.stringify(dados.festasAgendadas));
            
            if(dados.catalogoServicos) localStorage.setItem('serv_kenia_final', JSON.stringify(dados.catalogoServicos));
            if(dados.catalogoCombos) localStorage.setItem('combos_kenia_final', JSON.stringify(dados.catalogoCombos));
            if(dados.clientesSalvos) localStorage.setItem('cli_kenia_final', JSON.stringify(dados.clientesSalvos));
            
            alert('✅ Backup Restaurado com Sucesso! A página vai recarregar.'); location.reload();
        } catch (error) { alert('❌ Erro no arquivo de backup.'); }
    };
